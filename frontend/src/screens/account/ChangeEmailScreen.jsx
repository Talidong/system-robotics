...
import { getProfile, updateStoredUser } from '../../service/userStorage';
...
export default function ChangeEmailScreen({ navigation }) {
  ...
  const { loading, error, submitEmailChange } = useChangeEmail();
...
const handleSave = async () => {
  ...
  const success = await submitEmailChange({ currentEmail, newEmail, confirmEmail, sendToNew });

  if (success) {
    if (sendToNew) {
      navigation.navigate(ACCOUNT_ROUTES.CHANGE_EMAIL_VERIFICATION, { email: newEmail });
    } else {
      await updateStoredUser({ email: newEmail });
      navigation.navigate(ACCOUNT_ROUTES.CHANGE_EMAIL_SUCCESS, { email: newEmail });
    }
  }
};
  return (
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.btnRow}>
          ...
          <TouchableOpacity
            ...
            disabled={loading}
          >
            <Text style={styles.saveText}>{loading ? 'Sending...' : 'Save Changes'}</Text>
          </TouchableOpacity>
        </View>
      </ScrollView>
    </SafeAreaView>
